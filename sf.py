...
class SF():
    ...
    def get_lead_data(self, region):
        ...
        fields_txt = ', '.join(fields)
        query_text = f"SELECT {fields_txt} FROM Lead WHERE  (" \
                     f"Lead_Owner_Region__c in  " \
                     f"('US/CAN', 'LATAM') or Kitchen_Country__c in ('Mexico','Colombia','Brazil') )  and " \
                     f"(recordtypeid = '012f4000000Su90AAC' OR kitchen_type__c in ('Virtual', 'CloudRetail','virtual;cloudretail'))"
    ...
    def get_opportunity_data(self, region):
        ...
        query_text = f"SELECT {fields_txt} FROM Opportunity WHERE OwnerId in (SELECT Id FROM User WHERE Region__c in ('LATAM','US/CAN'))" \
                     f"AND Owner.ProfileId != '00ef4000001j4QJAAY' and" \
                     f" (recordtypeid = '012f4000000Su90AAC' OR kitchen_type__c in ('Virtual', 'CloudRetail','virtual;cloudretail'))"
